...
#if DDR_AUTO_DETECT
static uint32_t ma35d0_get_ddr_pid(void)
{
	return (mmio_read_32(SYS_BA) & MA35D0_DDR_PID_MASK) >> MA35D0_DDR_PID_SHIFT;
}

static void ma35d0_apply_ddr_setting(void)
{
	uint32_t pid;

	pid = ma35d0_get_ddr_pid();
	switch (pid) {
	MA35D0_DDR_CFG_LIST(APPLY_DDR_CFG)
	default:
